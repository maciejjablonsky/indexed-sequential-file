...
void commands::Interpreter::LoadCommands() {
    std::string command_line =
        std::visit(overloaded{[](commands::FileReader &file_reader) {
                                  return file_reader.ReadNextCommand();
                              },
                              [](commands::Prompt &prompt) {
                                  return prompt.GetNextCommand();
                              }},
                   source_);
...
commands::Interpreter::UnzipCommandLine(const std::string &command_line) const {
    ...
    std::visit(overloaded{[&](command_read &c) { ss >> c.key; },
                          ...
                          [&](command_update &c) { ss >> c.key >> c.record; },
                          [&](command_delete &c) { ss >> c.key; },
                          [](auto &c) {}},
               command);
    return command;
}
